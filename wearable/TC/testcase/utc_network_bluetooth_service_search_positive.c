...
void adapter_state_changed_cb_for_service_search_p(int result, bt_adapter_state_e adapter_state, void* user_data);
void device_discovery_state_changed_cb_for_service_search_p(int result, bt_adapter_device_discovery_state_e discovery_state,
		bt_adapter_device_discovery_info_s* discovery_info, void* user_data);
void service_searched_cb_for_service_search_p(int result, bt_device_sdp_info_s *sdp_info, void* user_data);
void device_bonded_cb_for_service_search_p(int result, bt_device_info_s *device_info, void *user_data);
...
static void utc_network_bluetooth_service_search_p(void)
{
	...
	{
		dts_fail("bt_device_start_service_search", "device discovery failed.");        
	}
	...
	if( bt_device_start_service_search(target_address) == BT_ERROR_NONE )
	{
		...
		g_main_loop_run(mainloop);
		g_source_remove(timeout_id);
		dts_check_eq("bt_device_start_service_search", callback_result, true, "Service search failed.");                
	}       
	else
	{
		dts_fail("bt_device_start_service_search", "bt_device_start_service_search() failed.");
	}
}
...
static void utc_network_bluetooth_service_search_cancel_p(void)
{
	...
	if( discovery_result != true )
	{
		dts_fail("bt_device_cancel_service_search", "device discovery failed.");        
	}

	callback_result = false;
	if( bt_device_start_service_search(target_address) != BT_ERROR_NONE )
	{
		dts_fail("bt_device_cancel_service_search", "bt_device_start_service_search() failed.");
	}
	else
	{
		tet_printf("bt_device_cancel_service_search", "bt_device_start_service_search() succeeded.");        
	}
	...
	{
		dts_fail("bt_device_cancel_service_search", "bt_device_cancel_service_search() failed.");
	}
}
