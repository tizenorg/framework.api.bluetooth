...
void adapter_state_changed_cb_for_discovery_p(int result, bt_adapter_state_e adapter_state, void* user_data);
void device_discovery_state_changed_cb_for_discovery_p(int result, bt_adapter_device_discovery_state_e discovery_state, 
		...
		bt_adapter_device_discovery_info_s *discovery_info, void* user_data)
{
	...
	else if( discovery_state == BT_ADAPTER_DEVICE_DISCOVERY_FINISHED )
	{
		...
		if( user_data != NULL )
		{
			if( (user_data != NULL) && !strcmp((char*)user_data, "start") && callback_result == false )
			{
...
static void utc_network_bluetooth_device_discovery_set_state_changed_cb_p(void)
{
	...
	ret = bt_adapter_set_device_discovery_state_changed_cb
		(device_discovery_state_changed_cb_for_discovery_p, "start");
	dts_check_eq("bt_adapter_set_device_discovery_state_changed_cb", ret, 
		BT_ERROR_NONE, "bt_adapter_set_device_discovery_state_changed_cb() failed.");
}
...
static void utc_network_bluetooth_device_discovery_start_p(void)
{
	...
	{       
		...
		g_main_loop_run(mainloop);	
		g_source_remove(timeout_id);
		dts_check_eq("bt_adapter_start_device_discovery", callback_result, true, 
			"Callback was called but failed.");
	}
	else
	{
		dts_fail("bt_adapter_start_device_discovery", "bt_adapter_start_device_discovery() failed.");
	}
}
...
static void utc_network_bluetooth_device_discovery_unset_state_changed_cb_p(void)
{
	...
	dts_check_eq("bt_adapter_unset_device_discovery_state_changed_cb", ret, 
		BT_ERROR_NONE, "bt_device_discovery_uset_state_changed_cb() failed");
}
...
static void utc_network_bluetooth_device_discovery_cancel_p(void)
{
	...
	{       
		...
		dts_check_eq("bt_adapter_stop_device_discovery", callback_result, true, "Callback was called but failed");
	}
	else
	{
		dts_fail("bt_adapter_stop_device_discovery", "bt_adapter_stop_device_discovery() failed");
	}
