...
/**
 * @ingroup CAPI_NETWORK_BLUETOOTH_DEVICE_MODULE
 * @brief  Called when the device gets disconnected.
 * @param[in] remote_address The address of the remote Bluetooth device
 * @param[in] disconn_reason The reason for device disconnection
 * @param[in] user_data The user data passed from the callback registration function
 * @see bt_device_set_disconnected_cb()
 * @see bt_device_unset_disconnected_cb()
 */
typedef void (*bt_device_disconnected_cb)(const char *remote_address,
			bt_device_disconnect_reason_e disconn_reason, void *user_data);
...
/**
 * @ingroup CAPI_NETWORK_BLUETOOTH_DEVICE_MODULE
 * @brief  Registers a callback function to be invoked when the device gets disconnected.
 * @param[in] callback The callback function to register
 * @param[in] user_data The user data to be passed to the callback function
 * @return 0 on success, otherwise a negative error value.
 * @retval #BT_ERROR_NONE  Successful
 ...
 * @retval #BT_ERROR_INVALID_PARAMETER  Invalid parameter
 * @pre The Bluetooth service must be initialized with bt_initialize().
 * @post bt_device_disconnected_cb() will be invoked.
 * @see bt_initialize()
 * @see bt_device_disconnected_cb()
 * @see bt_device_unset_disconnected_cb()
 */
int bt_device_set_disconnected_cb(bt_device_disconnected_cb callback, void *user_data);

/**
 * @ingroup CAPI_NETWORK_BLUETOOTH_DEVICE_MODULE
 * @brief	Unregisters the callback function to be invoked when the device disconnection happens.
 * @return 0 on success, otherwise a negative error value.
 * @retval #BT_ERROR_NONE  Successful
 * @retval #BT_ERROR_NOT_INITIALIZED  Not initialized
 * @pre The Bluetooth service must be initialized with bt_initialize().
 ...
 * @see bt_device_set_disconnected_cb()
 */
int bt_device_unset_disconnected_cb(void);
...
typedef struct {
	...
	float interval_max; /**< Maximum advertising interval for non-directed advertising.
			      A multiple of 0.625ms is only allowed (Time range : 20ms to 10.24sec). */
	int filter_policy; /* Advertising filter policy */
 ...
 * if you register this callback using bt_adapter_set_manufacturer_data_changed_cb().
 ...
 * @param[in] call_id  The call ID
 * @param[in] user_data The user data passed from the callback registration function
 * @see bt_hf_set_call_handling_event_cb()
 * @see bt_hf_unset_call_handling_event_cb()
 */
typedef void (*bt_hf_vendor_dep_cmd_event_cb) (bluetooth_vendor_dep_at_cmd cmd);
 ...
 * @pre The Bluetooth audio service must be initialized with bt_audio_initialize().
 * @see bt_audio_initialize()
 * @see bt_hf_call_handling_event_cb()
 * @see bt_hf_unset_call_handling_event_cb()
 */
...
/**
 * @ingroup CAPI_NETWORK_BLUETOOTH_AUDIO_HF_MODULE
 * @brief Gets the HF(Hands-Free) profile connected status.
