...
tc_table_t tc_table[] = {
	...
	{"bt_adapter_set_advertising_state_changed_cb"	, 20},
	{"bt_adapter_unset_advertising_state_changed_cb"	, 21},
	{"bt_adapter_start_advertising"	, 22},
	{"bt_adapter_stop_advertising"	, 23},
	{"bt_adapter_is_advertising"	, 24},
	{"bt_adapter_add_white_list"	, 25},
	{"bt_adapter_remove_white_list"	, 26},
	{"bt_adapter_clear_white_list"	, 27},
	{"bt_adapter_set_advertising_manufacturer_data_changed_cb"	, 28},
	{"bt_adapter_unset_advertising_manufacturer_data_changed_cb"	, 29},
	{"bt_adapter_set_advertising_manufacturer_data"	, 30},
	{"bt_adapter_get_advertising_manufacturer_data"	, 31},
	{"bt_adapter_set_advertising_manufacturer_data with zero length"	, 32},
	{"bt_adapter_set_connectable_changed_cb"	, 33},
	{"bt_adapter_unset_connectable_changed_cb"	, 34},
	{"bt_adapter_get_connectable"	, 35},
	{"bt_adapter_set_connectable"	, 36},
	...
	{"bt_panu_set_connection_state_changed_cb"		, 113},
	{"bt_panu_connect"	, 114},
	...
	{"bt_device_set_disconnected_cb" , 131},
	...
	{"bt_gatt_watch_characteristic_changes"	 , 150},
	{"bt_gatt_unwatch_characteristic_changes"	 , 151},
	...
	{"bluetooth_a2dp_set_content_protection(enable)"	, 170},
	{"bluetooth_a2dp_set_content_protection(disable)"	, 171},

	/* HF functions */
	{"bt_hf_initialize"	, 172},
	{"bt_hf_deinitialize"	, 173},
	{"bt_hf_notify_call_event(answer)"	, 174},
	{"bt_hf_notify_speaker_gain"	, 175},
	{"bt_hf_set_speaker_gain_changed_cb"	, 176},
	{"bt_hf_unset_speaker_gain_changed_cb"	, 177},
	{"bt_hf_notify_voice_recognition_state(true)"	, 178},
	{"bt_hf_notify_vendor_cmd"	, 179},
	{"bt_hf_notify_call_event(terminate)"	, 180},
	{"bt_hf_notify_call_event(initiate)"	, 181},
	{"bt_hf_notify_voice_recognition_state(false)"	, 182},
	{"bt_hf_set_sco_state_changed_cb()"	, 183},
	{"bt_hf_unset_sco_state_changed_cb()"	, 184},
	{"bt_hf_set_call_handling_event_cb()"	, 185},
	/* -----------------------------------------*/
	{"Finish"				, 0x00ff},
	{NULL					, 0x0000},
};
...
static void __bt_adapter_advertising_state_changed_cb(int result,
				bt_adapter_advertising_state_e adv_state,
				bt_adapter_advertising_params_s *adv_params,
				void *user_data)
{
	if (result != BT_ERROR_NONE) {
		TC_PRT("advertising state change is failed : [0x%04x]", result);
		return;
	}

	if (adv_state == BT_ADAPTER_ADVERTISING_STARTED) {
		TC_PRT("advertising is started "
				"(interval - min : %f, max : %f, filter: %x)",
				adv_params->interval_min,
				adv_params->interval_max,
				adv_params->filter_policy);
	} else
		TC_PRT("advertising is stopped");
}

static void __bt_adapter_advertising_manufacturer_data_changed_cb(char *data,
						int len, void *user_data)
{
	int i;

	TC_PRT("advertising manufacturer data is changed");
	for (i = 0; i < len; i++) {
...
{
	TC_PRT("event[%d] phone_number[%s]", event, phone_number);
}
...
void __bt_device_disconnected_cb(const char *remote_address,
						bt_device_disconnect_reason_e reason,
						void *user_data)
{
	TC_PRT("address: %s", remote_address);
	TC_PRT("Disconnect reason: %d", reason);
}
...
int test_input_callback(void *data)
{
	...
	case 20:
		ret = bt_adapter_set_advertising_state_changed_cb(__bt_adapter_advertising_state_changed_cb, NULL);
		if (ret < BT_ERROR_NONE)
			TC_PRT("failed with [0x%04x]", ret);
		break;

	case 21:
		ret = bt_adapter_unset_advertising_state_changed_cb();
		if (ret < BT_ERROR_NONE)
			TC_PRT("failed with [0x%04x]", ret);
		break;
	...
	case 24: {
		...
		if (ret < BT_ERROR_NONE)
			...
			TC_PRT("advertising is %s", is_advertising ? "started" : "stopped");
		break;
	}

	case 25: {
		ret = bt_adapter_add_white_list("11:22:33:44:55:66");
		if (ret < BT_ERROR_NONE)
			TC_PRT("failed with [0x%04x]", ret);
		break;
	}
	...
	case 26: {
		ret = bt_adapter_remove_white_list("11:22:33:44:55:66");
		if (ret < BT_ERROR_NONE)
			TC_PRT("failed with [0x%04x]", ret);
		break;
	}
	...
	case 28:
		ret = bt_adapter_set_advertising_manufacturer_data_changed_cb(__bt_adapter_advertising_manufacturer_data_changed_cb, NULL);
		if (ret < BT_ERROR_NONE)
			TC_PRT("failed with [0x%04x]", ret);
		break;

	case 29:
		ret = bt_adapter_unset_advertising_manufacturer_data_changed_cb();
		if (ret < BT_ERROR_NONE)
			TC_PRT("failed with [0x%04x]", ret);
		break;

	case 30: {
		char data[2] = {0x00, 0x75};
		ret = bt_adapter_set_advertising_manufacturer_data(data, sizeof(data));
		if (ret < BT_ERROR_NONE)
			TC_PRT("failed with [0x%04x]", ret);
		break;
	}
	...
	case 31: {
		...
		ret = bt_adapter_get_advertising_manufacturer_data(&data, &len);
		if (ret < BT_ERROR_NONE) {
			TC_PRT("failed with [0x%04x]", ret);
			break;
		}
	...
	case 32: {
		char data[0];
		ret = bt_adapter_set_advertising_manufacturer_data(data, 0);
		if (ret < BT_ERROR_NONE)
			TC_PRT("failed with [0x%04x]", ret);
		break;
	}
	...
	case 114: {
		...
		address = get_bd_from_file("/opt/addr-file");
		if (address == NULL)
			address = g_strdup("E4:32:CB:41:1D:7A");

		TC_PRT("CONNECTING TO %s", address);
		ret = bt_panu_connect(address, BT_PANU_SERVICE_TYPE_NAP);
		if (ret < BT_ERROR_NONE) {
			TC_PRT("failed with [0x%04x]", ret);
		}
	...
	case 131:
		ret = bt_device_set_disconnected_cb(__bt_device_disconnected_cb, NULL);
		if (ret < BT_ERROR_NONE) {
			TC_PRT("failed with [0x%04x]", ret);
		}
		break;
	...
	case 147: {
		...
		ret = bt_gatt_set_characteristic_value_request(characteristics[2], value, 1,
	...
	case 148 : {
		ret = bt_gatt_read_characteristic_value(characteristics[2], __bt_gatt_char_read_cb);
		if (ret < BT_ERROR_NONE) {
			TC_PRT("failed with [0x%04x]", ret);
		}
		break;
	...
	case 150: {
		ret = bt_gatt_watch_characteristic_changes(service_clone[1]);
		if (ret < BT_ERROR_NONE) {
			TC_PRT("failed with [0x%04x]", ret);
		}
		break;
	...
	case 151: {
		ret = bt_gatt_unwatch_characteristic_changes(service_clone[1]);
		if (ret < BT_ERROR_NONE) {
			TC_PRT("failed with [0x%04x]", ret);
		}
		break;
	...
	case 169:
		...
		break;
	case 170:
		ret = bt_a2dp_set_content_protection(TRUE);
		if (ret < BT_ERROR_NONE) {
			TC_PRT("failed with [0x%04x]", ret);
		}
		break;
	case 171:
		ret = bt_a2dp_set_content_protection(FALSE);
		if (ret < BT_ERROR_NONE) {
			TC_PRT("failed with [0x%04x]", ret);
		}
		break;
	...
	case 178:
		...
		break;
	case 179:
		ret = bt_hf_notify_vendor_cmd(1, "AT+XSAT=");
		if (ret < BT_ERROR_NONE) {
			TC_PRT("failed with [0x%04x]", ret);
		} else if (ret == BT_ERROR_NONE) {
			TC_PRT("Success");
